var MASlidingMenu = function(args) {
    var self = Ti.UI.createWindow({
        exitOnClose : false,
        backgroundImage : '/images/student_tutor_profile_popup/header_bg.png',
        fullscreen : true,
        navBarHidden : true,
        exitOnClose : true,
        titleControl : true,
    ...
    //first view would be rotating view
    ...
    var open = function() {
        ...
        for (var i = 0; i < left.data.length; i++) {
            ...
            for (var j = 0; j < left.data[i].rowCount; j++) {
                ...
                if (row.navView !== undefined) {
                    var View;
                    Ti.API.info(row.navView.toString().indexOf('Window') + "<---------");
                    ...
                    if (!view) {
                        ...
                        // set the first view we can find
                    }
        ...
        addEventListener('toggle', function() {
            if (current === 'view') {
                Ti.API.info("toggle triggered");
                slideView('left');
            } else {
                Ti.API.info("toggle view triggered");
                slideView('view');
            }
        });
        self.open({});
